...
// Background是最常用的，作为Context这个树结构的最顶层的Context，它不能被取消。当不知道用啥context的时候就可以用TODO。

// 要想结束所有线程，就调用ctx, cancel := context.WithCancel(context.Background())函数返回的cancel函数即可，
// 当撤销函数被调用之后，对应的Context值会先关闭它内部的接收通道，也就是它的Done方法返回的通道。
...
func TestCancelContext(t *testing.T) {
	...
	go func() {
		...
		cancel() //调用返回的cancel方法来让context生命周期结束
	}()
